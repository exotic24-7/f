...
class PetalContainer {
    ...
    draw(inGame, number) {
        ...
        if (this.petals[0].constructor === Petal) {
            if (!cachedImages.petals.indexed[this.type]) setPcIndexes(this.type);
            const imageIndex = cachedImages.petals.indexed[this.type][this.rarity];
            if (!this.nameless) {
                if(window.hqp){
                    ...
                    if (!cachedImages.petals.live[`${this.type}${imageIndex.index}`]) this.generatePetalImage(62.5, imageIndex);
                    const image = cachedImages.petals.live[`${this.type}${imageIndex.index}`];
                ...
                if (!cachedImages.petals.live[`${this.type}${imageIndex.index}nameless`]) this.generateNamelessPetalImage(62.5, imageIndex)
                const image = cachedImages.petals.live[`${this.type}${imageIndex.index}nameless`]
        ...
        if (this.petals[0].constructor === Enemy) {
            if (!cachedImages.enemies[`${this.type}`]) this.generateEnemyImage()
            const image = cachedImages.enemies[`${this.type}`]
        ...
        if (Colors.rarities[this.rarity].fancy !== undefined && window.hqp == true) {
            ...
            ctx.beginPath();
            ctx.rect(-29.5, -29.5, 59, 59);
            ctx.rect(21.5, -21.5, -44, 44);
            ctx.closePath();
...
let enemyOverrides = {
    ...
    "Fire Ant Burrow": {
        renderSize: 1.325,
        customRender: () => {
            enemyRenderMap['Fire Ant Burrow']({
                render: {
                    radius: 25,
                    angle: 0,
                    time: 5000000,
