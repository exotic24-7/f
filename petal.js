const petalRotationNaturalRotate = {
    ...
    "Sapphire": 1/1000,
    "Emerald": 1/1000,
    "Amulet of Divergence": 1/1000,
    "Amulet of Grace": 1/1000,
    "Amulet of Time": 1/1000,
    "Rice": 1/1000,
    "Yucca": 1/1000,
    "Clover": 1/1000, 
    "Horn": 1/500, 
    ...
    "Orange": 1/1000,
    "Mandible": 1/1000,
    "Blood Mandible": 1/1000,
    "Third Eye": 1/1000,
    "Bloodshot Eye": 1/1000,
    "Bubble": 1/1000,
    "Shiny Bubble": 1/1000,
    "Starfish": 1/1000,
    'Brisingida': 1/2000,
    ...
    "Trident": 1/500,
    "Card": 1/1000,
    "Waterlogged Compass": 1/1000,
    "Plank": 1/1000,
    "Fig": 1/1000,
    "Root": 1/1000,
    "Coconut": 1/500,
}
const petalRotationStickToParent = ["Spore", "Missile", "Dandelion", "Web", "Homing Missile", "Fire Missile", "Carrot", "Stalk"] //These petals are rotated to match to their rotation around parent flower
const petalFaceCenter = {
    "Stinger": 0,
    "Blood Stinger": 0,
    "Blade": 0,
    "Starfish": 0,
...
//^^ Such as stinger; each sub-particle faces center
...
class Petal {
    ...
    updateInterpolate(parent){
        if(this.miniPetalChildId !== undefined){
            ...
            parent.petalRotateSpeed = 0;
            parent.petalRotation = -f.petalRotation;
        }

        if (this.type == "Homing MissileProjectile"){
            if (this.lastPosition != undefined){
                this.render.selfAngle = Math.atan2(this.render.y - this.lastPosition.y, this.render.x - this.lastPosition.x);
            }
            this.lastPosition = {x: this.render.x, y: this.render.y};
        }
        ...
        this.render.angle = interpolateDirection(this.render.angle - parent.petalLag, this.angle, Math.min(0.8, Math.max(5 * Math.abs(parent.petalRotateSpeed), 0.3) * dt/16.66)) + parent.petalLag;
        if(this.dead === false && this.deadPosition !== undefined){
            delete this.deadPosition;
        }
        if(this.deadPosition !== undefined){
            this.render.x = interpolate(this.render.x, this.deadPosition.x, 0.26 * dt/16.66);
            this.render.y = interpolate(this.render.y, this.deadPosition.y, 0.26 * dt/16.66);
        } else {
        ...
        if (Object.keys(petalFaceCenter).includes(this.type)){
            this.selfAngle = petalFaceCenter[this.type] + this.render.angle + this.subStackedId/this.totalStackedId * Math.PI * 2;
        }
        if ((this.type === "Bone" && this.rarity >= 16)){
            this.selfAngle = this.render.angle + this.offset.angle;
        }
        if (this.type === 'Custom'){
            this.selfAngle = Math.atan2(this.render.y - parent.render.y, this.render.x - parent.render.x);
        }
        if (this.type === "Compass" || this.type === "Dark Compass" || this.type === "Waterlogged Compass") {
        ...
        // if (isNaN(this.render.x)) this.render.x = this.x
        // if (isNaN(this.render.y)) this.render.y = this.y
        // if (isNaN(this.render.angle)) this.render.angle = this.angle
        // if (isNaN(this.render.distance)) this.render.distance = this.distance
    }
    // simulate(parent){
    //     // ANGLE
    //     this.angle = parent.petalRotation + this.angleOffset//this.id / parent.petals.length * Math.PI * 2;
...
const petalRenderMap = {
    ...
    Shade: (p) => {
        ctx.fillStyle = blendColor('#111111', '#FF0000', blendAmount(p));
        if(checkForFirstFrame(p)){
            ctx.fillStyle = "#FFFFFF";
        }
        
        if (p.particles) if (!Array.isArray(p.particles)) p.particles = []
        if (!p.particles) p.particles = []

        let particleCount = 25

        if (p.particles.length < particleCount) for (let i = 0; i < particleCount; i++) {
            p.particles.push({
                distance: 0.25 * Math.random() * dt/16.6,
                angle: Math.PI * 2 * Math.random(),
                rotationSpeed: Math.random() * Math.PI / 16,
                size: 1.25 * Math.random() * .5
            })
        }

        for (let particle of p.particles) {
            particle.distance -= 0.05 * dt/16.6
            particle.size += 0.015 * dt/16.6
            particle.angle += particle.rotationSpeed * dt/16.6
            
            ctx.globalAlpha *= 1-particle.distance
            
            ctx.beginPath()
            ctx.arc(Math.cos(particle.angle) * particle.distance * p.radius * 2, Math.sin(particle.angle) * particle.distance * p.radius * 2, p.radius * particle.size, 0, Math.PI*2)
            ctx.fill()
            ctx.closePath()
            ...
            if (particle.distance < 0) {
                particle.distance = 0.5 + Math.random() * .5
                particle.angle = Math.PI * 2 * Math.random(),
                particle.rotationSpeed = Math.random() * Math.PI / 16,
                particle.size = 0.625 * Math.random() * .5
            }
        }
    ...
    Radiance: (p) => {
        ctx.fillStyle = blendColor('#eeeeee', '#FF0000', blendAmount(p));
        if(checkForFirstFrame(p)){
            ctx.fillStyle = "#FFFFFF";
        }
        ...
        for (let particle of p.particles) {
            ...
            if (particle.distance > 0.7) {
                particle.distance = 0.1 * Math.random() * dt/16.6
                particle.angle = Math.PI * 2 * Math.random(),
                particle.rotationSpeed = Math.random() * Math.PI / 16,
                particle.size = 1.25 * Math.random() * .5
            }
        }
    ...
    Horn: (p) => {
        ctx.lineWidth = 0.15 * p.radius;
        ctx.beginPath();
        let fill = blendColor('#af997e', '#FF0000', blendAmount(p));
		let stroke = blendColor('#8f7d67', '#FF0000', blendAmount(p));
		let shine = blendColor('#bdab95', '#FF0000', blendAmount(p));
		let depth = blendColor('#706150', '#FF0000', blendAmount(p));
        if(checkForFirstFrame(p)){
            fill = "#FFFFFF"; 
            stroke = "#FFFFFF";
			shine = "#FFFFFF"; 
		...
		ctx.fillStyle = fill
        ctx.beginPath();
        ctx.moveTo(1.16 * p.radius, -0.83 * p.radius);
		ctx.lineTo(0.54 * p.radius, -0.37 * p.radius);
		ctx.lineTo(-0.16 * p.radius, -0.23 * p.radius);
		ctx.lineTo(-0.86 * p.radius, -0.53 * p.radius);
		ctx.lineTo(-1.38 * p.radius, -0.99 * p.radius);
		ctx.lineTo(-1.23 * p.radius, -0.3 * p.radius);
		ctx.lineTo(-0.76 * p.radius, 0.22 * p.radius);
		ctx.lineTo(-0.18 * p.radius, 0.64 * p.radius);
		ctx.lineTo(0.52 * p.radius, 0.72 * p.radius);
		ctx.lineTo(1.28 * p.radius, 0.37 * p.radius);
        ctx.fill();
        ctx.closePath();

		ctx.strokeStyle = shine;
		ctx.beginPath();
		ctx.moveTo(-1.38 * p.radius, -0.99 * p.radius);
		ctx.lineTo(-0.91 * p.radius, -0.33 * p.radius);
		ctx.stroke();
		ctx.closePath();

		ctx.lineWidth = 0.16 * p.radius;
		ctx.beginPath();
		ctx.moveTo(-0.91 * p.radius, -0.33 * p.radius);
		ctx.lineTo(-0.27 * p.radius, 0.03 * p.radius);
		ctx.stroke();
		ctx.closePath();

		ctx.lineWidth = 0.18 * p.radius;
		ctx.beginPath();
		ctx.moveTo(-0.27 * p.radius, 0.03 * p.radius);
		ctx.lineTo(0.33 * p.radius, -0.03 * p.radius);
		ctx.stroke();
		ctx.closePath();

		ctx.lineWidth = 0.20 * p.radius;
		ctx.beginPath();
		ctx.moveTo(0.33 * p.radius, -0.03 * p.radius);
		ctx.lineTo(p.radius, -0.37 * p.radius);
		ctx.stroke();
		ctx.closePath();
		...
		ctx.beginPath();
		ctx.moveTo(1.16 * p.radius, -0.83 * p.radius)
		ctx.quadraticCurveTo(0.69 * p.radius, -0.15 * p.radius, 1.28 * p.radius, 0.37 * p.radius)
		ctx.quadraticCurveTo(1.67 * p.radius, -0.29 * p.radius, 1.16 * p.radius, -0.83 * p.radius)
		ctx.fill();
		ctx.stroke();
		ctx.closePath();

		ctx.beginPath();
		ctx.moveTo(-0.86 * p.radius, -0.53 * p.radius);
		ctx.quadraticCurveTo(-1.01 * p.radius, -0.26 * p.radius, -0.76 * p.radius, 0.22 * p.radius);
		ctx.moveTo(-0.16 * p.radius, -0.23 * p.radius);
		ctx.quadraticCurveTo(-0.53 * p.radius, 0.15 * p.radius, -0.18 * p.radius, 0.64 * p.radius);
		ctx.moveTo(0.54 * p.radius, -0.37 * p.radius);
		ctx.quadraticCurveTo(0.17 * p.radius, 0.25 * p.radius, 0.52 * p.radius, 0.72 * p.radius);
		ctx.stroke();
		ctx.closePath();
    ...
    "Blood Horn": (p) => {
        ctx.lineWidth = 0.15 * p.radius;
        ctx.beginPath();
        let fill = blendColor('#af7e7eff', '#FF0000', blendAmount(p));
		let stroke = blendColor('#8f6767ff', '#FF0000', blendAmount(p));
		let shine = blendColor('#bd9595ff', '#FF0000', blendAmount(p));
		let depth = blendColor('#705050ff', '#FF0000', blendAmount(p));
        if(checkForFirstFrame(p)){
            fill = "#FFFFFF"; 
            stroke = "#FFFFFF";
			shine = "#FFFFFF"; 
    ...
    "Abyssal Artifact": (p) => {
        ctx.lineWidth = 3;
        ctx.beginPath();
        ctx.fillStyle = blendColor('#2c1bd1', '#FF0000', blendAmount(p));
        ctx.strokeStyle = blendColor('#2718b8', '#FF0000', blendAmount(p));
        if(checkForFirstFrame(p)){
            ctx.fillStyle = "#FFFFFF"; 
            ctx.strokeStyle = "#FFFFFF";
        }
    ...
    Clover: (p) => {
        ...
        for (let i = 0; i < petalCount; i++) {
            ...
            ctx.beginPath()
            ctx.bezierCurveTo(0, 0, -0.5 * p.radius, -0.5 * p.radius, -0.3 * p.radius, -0.8 * p.radius)
            ctx.quadraticCurveTo(0, -1.15 * p.radius, 0.3 * p.radius, -0.8 * p.radius)
            ctx.bezierCurveTo(0.5 * p.radius, -0.5 * p.radius, 0, 0, 0, 0)
            ctx.closePath()
            ctx.fill()
            ctx.stroke()
            ctx.closePath()
    ...
    "Dark Spine": (p) => {
        ctx.strokeStyle = blendColor('#3a1f1f', '#1a0f0f', blendAmount(p));
        if (checkForFirstFrame(p)) {
            ctx.strokeStyle = "#ffffff"
        }
        ...
        ctx.quadraticCurveTo(p.radius * 0.05, 0, 0, p.radius * -1.2);
        ctx.stroke();
        ctx.closePath();
        
        ctx.strokeStyle = blendColor('#5a2d2d', '#2b1a1a', blendAmount(p));
        if (checkForFirstFrame(p)) {
        ...
        ctx.quadraticCurveTo(p.radius * -0.3, p.radius * 0.4, p.radius * -0.5, p.radius * 0.2);
        ctx.stroke();
        ctx.closePath();
        ...
        ctx.quadraticCurveTo(p.radius * -0.3, p.radius * -0.4, p.radius * -0.5, p.radius * -0.6);
        ctx.stroke();
        ctx.closePath();
    ...
    Cutter: (p) => {
        ...
        ctx.beginPath()
        ctx.arc(0, 0, p.radius, 0, Math.PI*2)
        ctx.rect(p.radius*2, -p.radius*2, -p.radius*4, p.radius*4)
        ctx.closePath()
    
        ctx.clip()
    
        ctx.rotate(Math.PI*time/650)
        ctx.beginPath();
        for (let i=0;i<9;i++) {
            ctx.rotate(i*Math.PI*2/8);
            ctx.quadraticCurveTo(0.39*p.radius, -0.9*p.radius, 1*p.radius, -1*p.radius)
            ctx.rotate(-i*Math.PI*2/8)
        }
        ctx.fill()
        ctx.rotate(-Math.PI*time/650)
    ...
    "Shiny Bubble": (p) => {
        ...
        if (p.particles.length < particleCount) if (p.particles.length < particleCount)  for (let i = 0; i < particleCount; i++) {
            p.particles.push({
                distance: 0.325 * Math.random(),
                angle: Math.PI * 2 * Math.random(),
                rotationSpeed: Math.random() * Math.PI / 16,
                size: 0.75 * Math.random() * .5
            })
        }

        for (let particle of p.particles) {
            particle.distance += 0.01 * dt/16.6
            particle.size -= 0.01 * dt/16.6
            particle.size = Math.max(0.05, particle.size)

            particle.angle += particle.rotationSpeed * dt/16.6
            
            ctx.globalAlpha *= (1-particle.distance)*0.75
            ...
            if (particle.distance > 0.3) {
                particle.distance = 0.1 * Math.random() * dt/16.6
                particle.angle = Math.PI * 2 * Math.random(),
                particle.rotationSpeed = Math.random() * Math.PI / 16,
                particle.size = 0.75 * Math.random() * .5
            }
        }

        ctx.fillStyle = blendColor('#fdff84', "#FF0000", Math.max(0, blendAmount(p)));
		ctx.strokeStyle = blendColor('#fdff84', "#FF0000", Math.max(0, blendAmount(p)));
		if (checkForFirstFrame(p)) {
			ctx.fillStyle = "#ffffff"; //"#FFFFFF";
			ctx.strokeStyle = "#ffffff" //'#cecece';//blendColor("#FFFFFF", "#000000", 0.19);
		}
		...
		ctx.globalAlpha *= 0.6 + ((Math.cos(Date.now()/50) + 1)/10);
		ctx.beginPath();
		ctx.arc(0, 0, p.radius * 9 / 10, 0, Math.PI * 2);
		ctx.stroke();
		ctx.closePath();
    ...
    Brisingida: (p) => {
        ...
        ctx.strokeStyle = blendColor('#3e87a9ff', '#FF0000', blendAmount(p));
        ctx.fillStyle = blendColor('#4db2d1ff', '#FF0000', blendAmount(p));
        if(checkForFirstFrame(p)){
            ctx.fillStyle = "#FFFFFF"; 
            ctx.strokeStyle = "#FFFFFF";
        }
        ...
        ctx.stroke();
        ctx.closePath();
        ctx.fillStyle = blendColor('#9c6cd4ff', '#FF0000', blendAmount(p))
        if(checkForFirstFrame(p)){
            ctx.fillStyle = "#FFFFFF"; 
        }
    ...
    "Waterlogged Compass": (p) => {
        ...
        if(p.glow > 0){
            ...
            let glowColor = `hsl(${(Date.now()/15)%360}, 50%, 50%)`
            let extraRadius = 0;
    ...
    "Waterlogged Dark Compass": (p) => {
        ctx.lineWidth = 2.4;

        
        ctx.beginPath();
        ctx.fillStyle = blendColor('#8f5830', '#FF0000', blendAmount(p));
        ctx.strokeStyle = blendColor('#404040', '#FF0000', blendAmount(p));
        let redCompass = blendColor('#c3c018', '#FF0000', blendAmount(p))
    ...
    "Shiny Lightning": (p) => {
        ctx.strokeStyle = blendColor('#b2b43b', '#FF0000', blendAmount(p));
        ctx.fillStyle = blendColor('#dcdf4a', '#FF0000', blendAmount(p));
        if(checkForFirstFrame(p)){
            ctx.fillStyle = "#ffffff"; 
            ctx.strokeStyle = "#ffffff";
        }
    ...
    "Blood Jolt": (p) => {
        ctx.strokeStyle = blendColor('#8a2525ff', '#FF0000', blendAmount(p));
        ctx.fillStyle = blendColor('#a82d2dff', '#FF0000', blendAmount(p));
        if(checkForFirstFrame(p)){
            ctx.fillStyle = "#ffffff"; 
            ctx.strokeStyle = "#ffffff";
        }
    ...
    "Jolt": (p) => {
        ctx.strokeStyle = blendColor('#00cc84', '#FF0000', blendAmount(p));
        ctx.fillStyle = blendColor('#00fca4', '#FF0000', blendAmount(p));
        if(checkForFirstFrame(p)){
            ctx.fillStyle = "#ffffff"; 
            ctx.strokeStyle = "#ffffff";
        }
    ...
    FlowerFace: (p) => {
        ...
        self.drawFlower(0, 0, p.radius);
    },
    "TanksmithProjectile": (p) => {
        ctx.beginPath();
        ctx.fillStyle = blendColor('#bfbfbf', '#FF0000', blendAmount(p));
        ctx.strokeStyle = blendColor('#868686', '#FF0000', blendAmount(p));
        if(checkForFirstFrame(p)){
            ctx.fillStyle = "#FFFFFF"; 
            ctx.strokeStyle = "#FFFFFF";
        }
    ...
    "Bloodshot Eye": (p) => {
        ctx.lineWidth = 0.1 * p.radius;
        ctx.beginPath();
        ctx.fillStyle = blendColor("#44173d", '#FF0000', blendAmount(p));
        ctx.strokeStyle = blendColor("#44173d", '#FF0000', blendAmount(p));
        if (checkForFirstFrame(p)) {
            ctx.fillStyle = "#FFFFFF";
            ctx.strokeStyle = "#FFFFFF"
        }
        ...
        ctx.quadraticCurveTo(p.radius * 0.75, 0, 0, p.radius * 0.9);
        ctx.quadraticCurveTo(-p.radius * 0.75, 0, 0, -p.radius * 0.9);
        ctx.fill();
        ctx.closePath();
        ctx.fillStyle = blendColor("#dbaaccff", '#FF0000', blendAmount(p));
    ...
    "Blood Mandible": (p) => {
        ...
        ctx.lineWidth = 3;
        ctx.beginPath();
        ctx.fillStyle = blendColor('#ec2d20ff', '#FF0000', blendAmount(p));
        ctx.strokeStyle = blendColor('#c50e01ff', '#FF0000', blendAmount(p));
        if(checkForFirstFrame(p)){
            ctx.fillStyle = "#FFFFFF"; 
            ctx.strokeStyle = "#FFFFFF";
        }
    ...
    Mandible: (p) => {
        ...
        ctx.lineWidth = 3;
        ctx.beginPath();
        ctx.fillStyle = blendColor('#761008', '#FF0000', blendAmount(p));
        ctx.strokeStyle = blendColor('#5a0c07', '#FF0000', blendAmount(p));
        if(checkForFirstFrame(p)){
            ctx.fillStyle = "#FFFFFF"; 
            ctx.strokeStyle = "#FFFFFF";
        }
    ...
    'Shiny Iris': (p) => {
        ctx.lineWidth = 3;
        ctx.beginPath();
        ctx.fillStyle = blendColor('#d4af5f', '#FF0000', blendAmount(p));
        ctx.strokeStyle = blendColor('#ab8d4c', '#FF0000', blendAmount(p));
        if(checkForFirstFrame(p)){
            ctx.fillStyle = "#FFFFFF";
            ctx.strokeStyle = "#FFFFFF";
        }
    ...
    Stalk: (p) => {
        ctx.fillStyle = blendColor('#3c763d', '#FF0000', blendAmount(p)); // green stalk
        ctx.strokeStyle = blendColor('#2b502b', '#FF0000', blendAmount(p));
        if (checkForFirstFrame(p)) {
            ctx.fillStyle = "#FFFFFF";
            ctx.strokeStyle = "#FFFFFF";
        }
        ...
        ctx.beginPath();
        // base of stalk at bottom
        ctx.moveTo(0, p.radius * 1); 
        // tip of stalk at top
        ctx.lineTo(-p.radius * 0.4, -p.radius); 
        ctx.lineTo(p.radius * 0.4, -p.radius); 
        ctx.lineTo(0, p.radius * 1); // back to base
        ctx.fill();
        ctx.stroke();
        ctx.closePath();
    },
    ...
    Spore: (p) => {
        ctx.fillStyle = blendColor('#9e7d5d', '#FF0000', blendAmount(p))
        ...
        ctx.beginPath();
        for (let i = 0; i <= 7; i++) {
            ctx.lineTo(Math.cos(i * Math.PI * 2 / 7) * p.radius * (Math.sin(i)/10+1), Math.sin(i * Math.PI * 2 / 7) * p.radius * (Math.sin(i+17.5)/10+1));
        }
        ctx.fill();
        ctx.stroke();
        ctx.closePath();
    ...
    Compost: (p) => {
        ctx.fillStyle = blendColor('#4b3b2b', '#ff0000ff', blendAmount(p))
        ...
        for (let i = 0; i <= segments; i++) {
            ...
            const noise = Math.sin(i * 1.7 + p.radius * 0.1) * 0.4
            const r = p.radius * (1 + noise)
        ...
        for (let i = 0; i < debrisCount; i++) {
            ...
            const dist = p.radius * (1.2 + Math.random() * 0.8)
            const size = p.radius * 0.1 * Math.random()
            ctx.beginPath()
    ...
    "Homing Missile": (p) => {
        let baseColor = blendColor("#222222", "#ff0000", blendAmount(p));
        let tipColor = blendColor("#325f28", "#ff0000", blendAmount(p));
        if (checkForFirstFrame(p)) {
            baseColor = "#FFFFFF";
            tipColor = "#FFFFFF";
        }
        ...
        ctx.lineWidth = p.radius / 1.5;
        ctx.moveTo(0, p.radius * 1.0);
        ctx.lineTo(p.radius * 0.9, p.radius * 0.3);
        ctx.lineTo(-p.radius * 0.9, p.radius * 0.3);
        ctx.closePath();
        ctx.fill();
        ctx.stroke();
        ...
        ctx.moveTo(0, -p.radius * 1.2);
        ctx.lineTo(p.radius * 0.5, p.radius * 0.3);
        ctx.lineTo(-p.radius * 0.5, p.radius * 0.3);
        ctx.closePath();
        ctx.fill();
        ctx.stroke();
    ...
    "Bloom": (p) => {
        ...
        for(let i = 10; i--; i>0){
            ctx.beginPath();
            ctx.arc(p.radius * Math.sin(i * 6.28318/10), p.radius * Math.cos(i * 6.28318/10), p.radius*0.7, 0, Math.PI*2);
            ctx.fill();
            ctx.stroke();
            ctx.closePath();    
        }
    ...
    BladeProjectile: (p) => {
        petalRenderMap.Blade(p);
    },
    "Homing MissileProjectile": (p) => {
        petalRenderMap["Homing Missile"](p);
    },
    "Fire MissileProjectile": (p) => {
        petalRenderMap["Fire Missile"](p);
    },
    "Bloodshot EyeProjectile": (p) => {
        petalRenderMap["Bloodshot Eye"](p);
    },
    "SporeProjectile": (p) => {
        petalRenderMap["Spore"](p);
    },
    ...
    BlueberriesProjectile: (p) => {
        petalRenderMap.Blueberries(p);
    },
    PomegranateProjectile: (p) => {
        petalRenderMap.Pomegranate(p);
    },
    PollenProjectile: (p) => {
    ...
    */
    ...
    "Blood RoseProjectile": (p) => {
        petalRenderMap["Blood Rose"](p);
    },

    DustProjectile: (p) => {
        petalRenderMap.Dust(p);
    },
    BloodJoltProjectile: (p) => {
        petalRenderMap["Blood Jolt"](p);
    },
    JoltProjectile: (p) => {
        petalRenderMap.Jolt(p);
    },
    DahliaProjectile: (p) => {
    ...
    NeurotoxinProjectile: (p) => {
        petalRenderMap.Neurotoxin(p);
    },
    ShellProjectile: (p) => {
        petalRenderMap.Shell(p);
    },
    NeurotoxinProjectilePuddle: (p) => {
        ctx.globalAlpha *= 0.2
        ctx.fillStyle = "#5353c9"
    ...
    WebProjectileWeb: (p) => {
        ...
        //     // ctx.stroke();
        //     // ctx.closePath();
		// 	// p.arc(0, 0, p.radius, 0, Math.PI * 2);
        // }
    ...
    Ruby: (p) => {
        ...
        if (p.rarity == 16 && window.noTrans !== true) {
            //ctx.fillStyle = blendColor("#ffffff", "#FF0000", blendAmount(p))

            ctx.save();

            ctx.beginPath();
            ctx.lineWidth = 0.3 * p.radius;
            ...
            ctx.fillStyle = blendColor("#5ccefa", "#FF0000", blendAmount(p))

            ctx.rotate(-p.selfAngle)
            ctx.beginPath();
            ctx.rect(-2 * p.radius, -1.05 * p.radius, 4 * p.radius, 0.6 * p.radius);
            ctx.rect(-2 * p.radius, 0.45 * p.radius, 4 * p.radius, 0.6 * p.radius);
            ctx.fill();
            ctx.closePath();

            ctx.fillStyle = blendColor("#f4a9b7", "#FF0000", blendAmount(p))

            ctx.beginPath();
            ctx.rect(-2 * p.radius, -0.45 * p.radius, 4 * p.radius, 0.3 * p.radius);
            ctx.rect(-2 * p.radius, 0.3 * p.radius, 4 * p.radius, 0.3 * p.radius);
            ctx.fill();
            ctx.closePath();
            ...
            ctx.beginPath();
            ctx.rect(-2 * p.radius, -0.15 * p.radius, 4 * p.radius, 0.45 * p.radius);
            ctx.fill();
            ctx.closePath();
    ...
    "Shiny Ruby": (p) => {
        ctx.beginPath();
        ctx.fillStyle = blendColor("#e0e03f", '#FF0000', blendAmount(p));
        ctx.strokeStyle = blendColor("#9fac27", '#FF0000', blendAmount(p));
        ctx.lineWidth = 3;
        if (checkForFirstFrame(p)) {
            ctx.fillStyle = "#FFFFFF";
            ctx.strokeStyle = "#FFFFFF"
        ...
        ctx.closePath();
        ctx.beginPath();
        ctx.fillStyle = blendColor(blendColor("#e9e174", '#ffffff', 0.3), '#FF0000', blendAmount(p));
        if (checkForFirstFrame(p)) {
            ctx.fillStyle = "#FFFFFF";
        }
        ctx.rotate(Math.PI / 4)
    ...
    Sapphire: (p) => {
        ...
        */
        ...
        ctx.rotate(Math.PI / 4)
        for (let i = 0; i <= 8; i++) {
            ctx.lineTo(Math.cos(i * Math.PI * 2 / 8) * p.radius, Math.sin(i * Math.PI * 2 / 8) * p.radius);
        }
        ctx.rotate(-Math.PI / 4)
        ctx.fill();
        ctx.stroke();
        ...
        ctx.rotate(Math.PI / 4)
        for (let i = 0; i <= 8; i++) {
            ctx.lineTo(Math.cos(i * Math.PI * 2 / 8) * p.radius * 0.6, Math.sin(i * Math.PI * 2 / 8) * p.radius * 0.6);
        }
        ctx.rotate(-Math.PI / 4)
        ctx.fill();
        ctx.closePath();
    ...
    "Amulet of Divergence": (p) => {

        // main border

        ctx.fillStyle = blendColor("#ffd256", "#ff0000", blendAmount(p));
        ctx.strokeStyle = blendColor("#d1ac46", "#ff0000", blendAmount(p));
        ...
        for (let i = 0; i <= sides; i++) {
            ctx.lineTo(Math.cos(i*angle) * p.radius, Math.sin(i*angle) * p.radius);
        };
        ctx.fill();
        ctx.stroke();
        ctx.closePath();
        ...
        ctx.fillStyle = p.rarity < 11 ? blendColor("#393939", "#ff0000", blendAmount(p)) : p.rarity < 12 ? blendColor("#632899", "#ff0000", blendAmount(p)) : p.rarity > 12 ? blendColor("#7e52a7", "#ff0000", blendAmount(p)) : blendColor("#7123b9", "#ff0000", blendAmount(p));
        ...
        ctx.moveTo(p.radius, 0);
        for (let i = 0; i <= sides; i++) {
            ctx.lineTo(Math.cos(i*angle) * p.radius / 2, Math.sin(i*angle) * p.radius / 2);
        };
        ctx.fill();
        ctx.closePath();

        ctx.fillStyle = p.rarity < 11 ? blendColor("#464646", "#ff0000", blendAmount(p)) : p.rarity < 12 ? blendColor("#7931bb", "#ff0000", blendAmount(p)) : p.rarity > 12 ? blendColor("#9a64cc", "#ff0000", blendAmount(p)) : blendColor("#8A2BE2", "#ff0000", blendAmount(p));
        ...
        ctx.moveTo(p.radius, 0)
        for (let i = 0; i <= sides; i++) {
            ctx.lineTo(Math.cos(i*angle) * p.radius / 4, Math.sin(i*angle) * p.radius / 4);
        };
        ctx.fill();
        ctx.closePath();
    },
    "Shard of Divergence": (p) => {
        ctx.fillStyle = blendColor("#8A2BE2", '#FF0000', blendAmount(p));
        ctx.strokeStyle = blendColor("#7123b9", '#FF0000', blendAmount(p));
        ctx.lineWidth = 0.2 * p.radius;
        ...
        ctx.fillStyle = blendColor(blendColor(ctx.fillStyle, '#ffffff', 0.18), '#FF0000', blendAmount(p));
        if (checkForFirstFrame(p)) {
            ctx.fillStyle = "#FFFFFF";
        }

        ctx.beginPath();
        ctx.moveTo(0, p.radius * 0.9);
    ...
    "Amulet of Grace": (p) => {
        ...
        ctx.fillStyle = p.rarity < 11 ? blendColor("#393939", "#ff0000", blendAmount(p)) : p.rarity < 12 ? blendColor("#aa7e84", "#ff0000", blendAmount(p)) : blendColor("#e8a5af", "#ff0000", blendAmount(p));
        ...
        ctx.moveTo(p.radius, 0)
        for (let i = 0; i <= sides; i++) {
            ctx.lineTo(Math.cos(i*angle) * p.radius / 1.5, Math.sin(i*angle) * p.radius / 1.5);
        };
        ctx.fill();
        ctx.closePath();

        ctx.fillStyle = p.rarity < 11 ? blendColor("#464646", "#ff0000", blendAmount(p)) : p.rarity < 12 ? blendColor("#d09aa2", "#ff0000", blendAmount(p)) : blendColor("#FFB6C1", "#ff0000", blendAmount(p));
        ...
        ctx.moveTo(p.radius, 0)
        for (let i = 0; i <= sides; i++) {
            ctx.lineTo(Math.cos(i*angle) * p.radius / 3, Math.sin(i*angle) * p.radius / 3);
        };
        ctx.fill();
        ctx.closePath();
    },
    "Shard of Grace": (p) => {
        ctx.fillStyle = blendColor("#FFB6C1", '#FF0000', blendAmount(p));
        ctx.strokeStyle = blendColor("#e8a5af", '#FF0000', blendAmount(p));
        ctx.lineWidth = 0.2 * p.radius;
        ...
        ctx.fillStyle = blendColor(blendColor(ctx.fillStyle, '#ffffff', 0.25), '#FF0000', blendAmount(p));
        if (checkForFirstFrame(p)) {
            ctx.fillStyle = "#FFFFFF";
        }
    ...
    "Amulet of Time": (p) => {
        ...
        ctx.fillStyle = p.rarity < 11 ? blendColor("#393939", "#ff0000", blendAmount(p)) : p.rarity < 12 ? blendColor("#11acae", "#ff0000", blendAmount(p)) : blendColor("#00CED1", "#ff0000", blendAmount(p));
        ...
        ctx.fillStyle = p.rarity < 11 ? blendColor("#464646", "#ff0000", blendAmount(p)) : p.rarity < 12 ? blendColor("#33b2b4", "#ff0000", blendAmount(p)) : blendColor("#2dd6d9", "#ff0000", blendAmount(p));
        ...
        ctx.strokeStyle = "#ffffff";
        ctx.globalAlpha = p.rarity < 11 ? 0.25 : 0.5;
        ctx.beginPath();
        ctx.moveTo(0, 0);
        ctx.lineTo(0.25 * p.radius, 0);
        ctx.moveTo(0, 0)
        ctx.lineTo(0, -0.5 * p.radius);
        ctx.stroke();
        ctx.closePath();
        ctx.globalAlpha = 1;
    },
    "Shard of Time": (p) => {
        ctx.fillStyle = blendColor("#2dd6d9", '#FF0000', blendAmount(p));
        ctx.strokeStyle = blendColor("#00CED1", '#FF0000', blendAmount(p));
        ctx.lineWidth = 0.2 * p.radius;
    ...
    "Amulet of GraceProjectile": (p) => {
        petalRenderMap["Amulet of Grace"](p);
    },
    "Shard of GraceProjectile": (p) => {
        petalRenderMap["Shard of Grace"](p);
    },
    Emerald: (p) => {
        ctx.beginPath();
        ...
        ctx.rotate(Math.PI / 4)
        for (let i = 0; i <= 5; i++) {
            ctx.lineTo(Math.cos(i * Math.PI * 2 / 5) * p.radius * 0.6, Math.sin(i * Math.PI * 2 / 5) * p.radius * 0.6);
        }
        ctx.rotate(-Math.PI / 4)
        ctx.fill();
        ctx.closePath();
    ...
    Powder: (p) => {
        if (p.rarity == 16 && window.noTrans !== true) {
            ...
            ctx.rotate(-p.selfAngle)
            ctx.beginPath();
            ctx.rect(-2 * p.radius, -1.4 * p.radius, 4 * p.radius, 0.7 * p.radius);
            ctx.rect(-2 * p.radius, 0.6 * p.radius, 4 * p.radius, 0.8 * p.radius);
            ctx.fill();
            ctx.closePath();
            ...
            ctx.beginPath();
            ctx.rect(-2 * p.radius, -0.8 * p.radius, 4 * p.radius, 0.6 * p.radius);
            ctx.rect(-2 * p.radius, 0.2 * p.radius, 4 * p.radius, 0.6 * p.radius);
            ctx.fill();
            ctx.closePath();
    ...
    Dust: (p) => {
        ctx.fillStyle = blendColor("#888888", "#FF0000", blendAmount(p))
        const time = Date.now() * 0.05; // slow down time
        const numParticles = 20;
    
        for (let i = 0; i < numParticles; i++) {
            const baseAngle = (i / numParticles) * Math.PI * 2;
            const angle = baseAngle + Math.sin(time * 0.3 + i) * 0.5;
            const radiusOffset = Math.cos(time * 0.2 + i * 2) * p.radius * 0.2;
            const distance = p.radius * 0.5 + radiusOffset;
    
            const x = Math.cos(angle) * distance;
            const y = Math.sin(angle + i * 0.5) * (distance * 0.8);
    
            const size = p.radius * 0.2 + Math.sin(time * 0.7 + i * 3) * p.radius * 0.05;
        ...
        // Slightly pulsating central cluster
        const coreSize = p.radius * 0.3 + Math.sin(time * 0.5) * p.radius * 0.03;
        ctx.beginPath();
    ...
    Blade: (p) => {
        ...
        ctx.lineWidth = p.radius / divCoef / 6;
        ctx.fillStyle = blendColor('#3ba43b', '#FF0000', blendAmount(p));
        ctx.strokeStyle = blendColor('#1f5c1f', '#FF0000', blendAmount(p));
        if (checkForFirstFrame(p)) {
            ctx.fillStyle = "#FFFFFF";
            ctx.strokeStyle = "#FFFFFF";
        }
        ...
        // left side
        ctx.moveTo(0, p.radius / divCoef * 2.2);
        ctx.quadraticCurveTo(-0.5 * p.radius / divCoef, -0.5 * p.radius / divCoef, 0, -2.8 * p.radius / divCoef);
        ...
        // central vein
        ctx.beginPath();
        ctx.moveTo(0, p.radius / divCoef * 2.1);
        ctx.lineTo(0, -2.6 * p.radius / divCoef);
        ctx.stroke();
        ctx.closePath();
    },
    Cinderleaf: (p) => {
        ...
        ctx.lineWidth = p.radius/divCoef/2.5//2.2;
        // ctx.beginPath();
        ctx.fillStyle = blendColor('#fc9547', '#FF0000', blendAmount(p));
        ctx.strokeStyle = blendColor('#cc7b3d', '#FF0000', blendAmount(p));
        if(checkForFirstFrame(p)){
            ctx.fillStyle = "#FFFFFF";
            ctx.strokeStyle = "#FFFFFF";
        }
    ...
    "Blood Leaf": (p) => {
        ...
        ctx.lineWidth = p.radius/divCoef/2.5//2.2;
        // ctx.beginPath();
        ctx.strokeStyle = blendColor('#b73333', '#FF0000', blendAmount(p));
        ctx.fillStyle = blendColor('#e03f3f', '#FF0000', blendAmount(p));
        if(checkForFirstFrame(p)){
            ctx.fillStyle = "#FFFFFF";
            ctx.strokeStyle = "#FFFFFF";
    ...
    Toxin: (p) => {
        ...
        ctx.fillStyle = blendColor('#8e44ad', '#00FF00', blendAmount(p)); // toxic purple to green
        ctx.strokeStyle = blendColor('#6c3483', '#00FF00', blendAmount(p));
        ...
        const startX = Math.cos(0) * p.radius * 1.2;
        const startY = Math.sin(0) * p.radius * 1.2;
        ctx.moveTo(startX, startY);
        
        // Outer spiky loop
        for (let i = 0; i <= Math.PI * 2; i += Math.PI / 6) {
            const angleOffset = Math.PI / 12;
            const spikiness = i % (Math.PI / 3) === 0 ? 1.2 : 0.8;
            const cpR = p.radius * spikiness;
            const cpX = Math.cos(i - angleOffset) * (cpR * 0.9);
            const cpY = Math.sin(i - angleOffset) * (cpR * 0.9);
            const endX = Math.cos(i) * cpR;
            const endY = Math.sin(i) * cpR;
        ...
        // Inner core
        ctx.fillStyle = blendColor('#b77fe3', '#00FF00', blendAmount(p));
        if (checkForFirstFrame(p)) {
            ctx.fillStyle = "#FFFFFF";
        }
        ctx.beginPath();
        ...
        // Toxic spikes around edges
        ctx.strokeStyle = blendColor('#ffffff', '#00FF00', blendAmount(p));
        if (checkForFirstFrame(p)) {
            ctx.strokeStyle = "#FFFFFF";
        }
        ctx.lineWidth = 1;
        for (let i = 0; i < 8; i++) {
            ...
            const x1 = Math.cos(angle) * (p.radius - 6);
            const y1 = Math.sin(angle) * (p.radius - 6);
            const x2 = Math.cos(angle) * (p.radius + 6);
            const y2 = Math.sin(angle) * (p.radius + 6);
            ctx.beginPath();
            ctx.moveTo(x1, y1);
            ctx.lineTo(x2, y2);
            ctx.stroke();
    ...
    Neurotoxin: (p) => {
        ...
        ctx.lineWidth = p.radius / divCoef / 5.5;
        ctx.fillStyle = blendColor('#4374ad', '#00FF00', blendAmount(p)); // toxic purple to green
        ctx.strokeStyle = blendColor('#335883', '#00FF00', blendAmount(p));
        ...
        // Inner core
        ctx.fillStyle = blendColor('#7fbbe2', '#00FF00', blendAmount(p));
        if (checkForFirstFrame(p)) {
            ctx.fillStyle = "#FFFFFF";
        }
        ctx.beginPath();
        ...
        // Toxic spikes around edges
        ctx.strokeStyle = blendColor('#cccccc', '#00FF00', blendAmount(p));
        if (checkForFirstFrame(p)) {
            ctx.strokeStyle = "#FFFFFF";
        }
        ctx.lineWidth = 1;
    ...
    Batrachotoxin: (p) => {
        ...
        ctx.lineWidth = p.radius / divCoef / 5.5;
        ctx.fillStyle = blendColor('#ad6c43', '#00FF00', blendAmount(p)); // toxic purple to green
        ctx.strokeStyle = blendColor('#835233', '#00FF00', blendAmount(p));
        ...
        // Inner core
        ctx.fillStyle = blendColor('#e2967f', '#00FF00', blendAmount(p));
        if (checkForFirstFrame(p)) {
            ctx.fillStyle = "#FFFFFF";
        }
        ctx.beginPath();
        ...
        // Toxic spikes around edges
        ctx.strokeStyle = blendColor('#aaaaaa', '#00FF00', blendAmount(p));
        if (checkForFirstFrame(p)) {
            ctx.strokeStyle = "#FFFFFF";
        }
        ctx.lineWidth = 1;
    ...
    "Rose": (p) => {
        ...
        if (p.rarity >= 12){
            ...
            ctx.beginPath()
            ctx.moveTo(Math.cos(-0.5 * angle) * p.radius * 0.9, Math.sin(-0.5 * angle) * p.radius * 0.9)
            for (let i = 0; i < sides; i++) {
                ctx.quadraticCurveTo(Math.cos(angle * i) * p.radius * 2, Math.sin(angle * i) * p.radius * 2, Math.cos(angle * (i + .5)) * p.radius * 0.9, Math.sin(angle * (i + .5)) * p.radius * 0.9)
            }
            //ctx.closePath()
            ctx.fill()
            ctx.stroke()
    ...
    "Blood Corn": (p) => {
        ctx.lineWidth = p.radius*0.65
        // ctx.beginPath();
        ctx.fillStyle = blendColor('#ff1919', '#FF0000', blendAmount(p));
        ctx.strokeStyle = blendColor('#cf1414', '#FF0000', blendAmount(p));
        if(checkForFirstFrame(p)){
            ctx.fillStyle = "#FFFFFF";
            ctx.strokeStyle = "#FFFFFF";
        }
    ...
    "Shiny Wing": (p) => {
        ...
        ctx.lineWidth = p.radius/divCoef/1.9//2.2;
        // ctx.beginPath();
        ctx.fillStyle = blendColor('#fff991', '#FF0000', blendAmount(p));
        ctx.strokeStyle = blendColor('#d1cc76', '#FF0000', blendAmount(p));
        if(checkForFirstFrame(p)){
            ctx.fillStyle = "#FFFFFF";
            ctx.strokeStyle = "#FFFFFF";
        }
    ...
    "Coconut": (p) => {
        ...
        ctx.lineWidth = p.radius / divCoef / 2.2;
        ctx.fillStyle = blendColor('#8b5a2b', '#FF0000', blendAmount(p)); // Medium brown
        ctx.strokeStyle = blendColor('#5c3b1a', '#FF0000', blendAmount(p)); // Darker outline
        if (checkForFirstFrame(p)) {
            ctx.fillStyle = "#FFFFFF";
            ctx.strokeStyle = "#FFFFFF";
        }
        ...
        ctx.fillStyle = blendColor('#3b2412', '#FF0000', blendAmount(p));
        if (checkForFirstFrame(p)) {
            ctx.fillStyle = "#FFFFFF";
        }

        const eyeRadius = p.radius / 6;

        for (let i = -1; i <= 1; i++) {
            ctx.beginPath();
            ctx.arc(p.radius * 0.35 * Math.cos(i * 2.0943), p.radius * 0.35 * Math.sin(i * 2.0943), eyeRadius, 0, Math.PI * 2);
            ctx.fill();
            ctx.closePath();
        }
    },
    "Fig": (p) => {
        ...
        ctx.lineWidth = p.radius / divCoef / 2.2;
        ctx.fillStyle = blendColor('#7d3c98', '#FF0000', blendAmount(p)); 
        ctx.strokeStyle = blendColor('#5e2b7b', '#FF0000', blendAmount(p));
        if (checkForFirstFrame(p)) {
            ctx.fillStyle = "#FFFFFF";
            ctx.strokeStyle = "#FFFFFF";
        }
        ...
        ctx.moveTo(0, -p.radius * 0.9); 
        ctx.bezierCurveTo(
            p.radius * 0.9, -p.radius * 0.5,
            p.radius * 0.9, p.radius * 0.9,
            0, p.radius
        );
        ctx.bezierCurveTo(
            -p.radius * 0.9, p.radius * 0.9,
    ...
    "Blood Oranges": (p) => {
        ...
        ctx.lineWidth = p.radius/divCoef/2.2//2.2;
        // ctx.beginPath();
        ctx.fillStyle = blendColor('#f04848', '#FF0000', blendAmount(p));
        ctx.strokeStyle = blendColor('#c23a3a', '#FF0000', blendAmount(p));
        if(checkForFirstFrame(p)){
            ctx.fillStyle = "#FFFFFF";
            ctx.strokeStyle = "#FFFFFF";
        }
    ...
    "Spikes": (p) => {
        ...
        // Base colors (body of petal)
        ctx.fillStyle = blendColor('#f5d76e', '#ff5e5e', blendAmount(p));
        ctx.strokeStyle = blendColor('#c79e2b', '#ff0000', blendAmount(p));
        if(checkForFirstFrame(p)){
            ctx.fillStyle = "#FFFFFF";
            ctx.strokeStyle = "#FFFFFF";
        }
    ...
    "Pomegranate": (p) => {
        ...
        ctx.lineWidth = p.radius/divCoef/2.5//2.2;
        // ctx.beginPath();
        ctx.strokeStyle = blendColor("#cf235f", '#FF0000', blendAmount(p));
        ctx.fillStyle = blendColor("#ff2b75", '#FF0000', blendAmount(p));
        if(checkForFirstFrame(p)){
            ctx.fillStyle = "#FFFFFF";
            ctx.strokeStyle = "#FFFFFF";
        }
    ...
    "Shiny Cactus": (p) => {
        ...
        ctx.lineWidth = p.radius/divCoef/5.5//2.2;
        // ctx.beginPath();
        ctx.fillStyle = blendColor("#d7d228", '#FF0000', blendAmount(p));
        ctx.strokeStyle = blendColor("#aeaa20", '#FF0000', blendAmount(p));
        if(checkForFirstFrame(p)){
            ctx.fillStyle = "#FFFFFF";
            ctx.strokeStyle = "#FFFFFF";
        }
        ...
        ctx.fillStyle = blendColor('#f1ee59', '#FF0000', blendAmount(p));
        if(checkForFirstFrame(p)){
            ctx.fillStyle = "#FFFFFF";
        }
        ctx.beginPath();
    ...
    "Shiny Egg": (p) => {
		...
		ctx.fillStyle = blendColor('#f5fa6e', "#FFFFFF", Math.max(0, blendAmount(p)));
		ctx.strokeStyle = blendColor('#c6ca59', "#FFFFFF", Math.max(0, blendAmount(p)));
		if(checkForFirstFrame(p)){
            ctx.fillStyle = "#FFFFFF"; 
            ctx.strokeStyle = "#FFFFFF";
        }
    ...
    "Neuroflare Egg": (p) => {
        ctx.lineWidth = 3;
        ctx.beginPath();
        ctx.fillStyle = blendColor('#44b8b4', '#FF0000', blendAmount(p));
        ctx.strokeStyle = blendColor('#2c9490', '#FF0000', blendAmount(p));
        if(checkForFirstFrame(p)){
            ctx.fillStyle = "#FFFFFF"; 
            ctx.strokeStyle = "#FFFFFF";
        }
    ...
    Magnet: (p) => {
        ...
        if (p.extraRot) {
            ctx.rotate(p.extraRot);
            ctx.strokeStyle = Colors.rarities[p.magC].color;
            ctx.lineWidth = p.radius * Math.max(0, Math.min(0.25, p.extraRot - 0.1))

            ctx.beginPath();
            ctx.arc(0, 0, p.radius * (2.5 + Math.cos(p.extraRot + Math.PI)) * 0.75, 0, Math.PI * 2 );
            ctx.stroke();
            ctx.closePath();
        }
        ...
        if (p.extraRot) {
            ...
            p.extraRot *= 0.98
            if (p.extraRot < 0.1) delete p.extraRot
        }
    },
    Root: (p) => {
        // Outer root shape (dark brown)
        ctx.lineWidth = p.radius * 0.2;
        ctx.fillStyle = blendColor('#b96c32', '#FF0000', blendAmount(p)); // Dark brown
        ctx.strokeStyle = blendColor('#965728', '#FF0000', blendAmount(p)); // Even darker outline
        if (checkForFirstFrame(p)) {
            ctx.fillStyle = "#FFFFFF";
            ctx.strokeStyle = "#FFFFFF";
        }
        ...
        ctx.beginPath();
        ctx.moveTo(p.radius * -0.45, p.radius * -1.18);
        ctx.lineTo(p.radius * -0.3, p.radius * -0.94);
        ctx.lineTo(p.radius * 0.25, p.radius * -0.72);
        ctx.lineTo(p.radius * 0.09, p.radius * 0.17);
        ctx.lineTo(p.radius * 0.77, p.radius * 0.89);
        ctx.lineTo(p.radius * 0.30, p.radius * 0.95);
        ctx.lineTo(p.radius * -0.11, p.radius * 1.17);
        ctx.lineTo(p.radius * -0.44, p.radius * 0.12);
        ctx.lineTo(p.radius * -0.14, p.radius * -0.55);
        ctx.lineTo(p.radius * -0.48, p.radius * -0.9);
        ctx.closePath();
        ctx.fill();
        ctx.stroke();
    },
    ...
    Square: (p) => {
        ...
        let size = 1.3 + 0.1 * Math.cos(time / 300);

        ctx.globalAlpha *= 0.5

        ctx.beginPath();
        for(let i = 0; i <= 4; i++){
			ctx.lineTo(Math.cos(i * 1.57079) * p.radius * size, Math.sin(i * 1.57079) * p.radius * size);
        }
		ctx.fill();
        ctx.closePath();
    ...
    Pentagon: (p) => {
        ...
        ctx.beginPath();
        for(let i = 0; i <= 5; i++){
			ctx.lineTo(Math.cos(i * 1.256632) * p.radius * size, Math.sin(i * 1.256632) * p.radius * size);
        }
		ctx.fill();
        ctx.closePath();
    ...
    Hexagon: (p) => {
		...
		ctx.fillStyle = blendColor('#b50e11', '#FF0000', blendAmount(p));
		ctx.strokeStyle = blendColor('#80090b', '#FF0000', blendAmount(p));
        if (checkForFirstFrame(p)) {
            ctx.fillStyle = "#FFFFFF";
            ctx.strokeStyle = "#FFFFFF"
        }
        ...
        ctx.beginPath();
        for (let i = 0; i <= 6; i++) {
            ctx.lineTo(Math.cos(i * Math.PI * 2 / 6) * p.radius * size, Math.sin(i * Math.PI * 2 / 6) * p.radius * size)
        }
        ctx.lineTo(p.radius, 0)
        ctx.fill();
        ctx.closePath();
    ...
    Cash: (p) => {
        ...
        ctx.beginPath()
        ctx.rect(-p.radius * 0.75, -p.radius * 0.375, p.radius * 1.5, p.radius * 0.75)
        ctx.fill()
        ctx.stroke()
        ctx.closePath()

        ctx.fillStyle = blendColor('#2e933c', '#FF0000', blendAmount(p));
        ctx.beginPath()
        ctx.arc(0, 0, 0.2 * p.radius, 0, Math.PI*2)
        ctx.fill()
        ctx.closePath()
        
        ctx.rect(-p.radius * 0.525, -p.radius * 0.2, p.radius * 0.1, p.radius * 0.4)
        ctx.fill()
        ctx.stroke()
        ctx.closePath()

        ctx.rect(p.radius * 0.425, -p.radius * 0.2, p.radius * 0.1, p.radius * 0.4)
        ctx.fill()
        ctx.stroke()
        ctx.closePath()
    },
    ...
    Lilypad: (p) => {
		ctx.lineWidth = p.radius * 0.15
		let colorA = blendColor('#39b54a', '#FF0000', blendAmount(p));
		let colorB = blendColor('#2e933c', '#FF0000', blendAmount(p));
		let colorC = blendColor('#33a443', '#FF0000', blendAmount(p));
		if (checkForFirstFrame(p)) {
			colorA = "#FFFFFF";
			colorB = "#FFFFFF";
			colorC = "#FFFFFF";
		...
		ctx.beginPath();
		ctx.moveTo(0, 0)
		ctx.arc(0, 0, p.radius, -Math.PI / 8, Math.PI / 8, true);
		ctx.lineTo(0, 0)
		ctx.fill();
		ctx.closePath();
		...
		ctx.beginPath();
		ctx.moveTo(0, 0)
		ctx.arc(0, 0, p.radius * 0.66, -Math.PI / 8, Math.PI / 8, true);
		ctx.lineTo(0, 0)
		ctx.fill();
		ctx.closePath();
    ...
    "Flowering Lilypad": (p) => {
		...
		let colorB = blendColor('#2e933c', '#FF0000', blendAmount(p));
		let colorC = blendColor('#33a443', '#FF0000', blendAmount(p));
		let blossomA = blendColor('#ff94c9', '#FF0000', blendAmount(p));
		let blossomB = blendColor('#cf78a3', '#FF0000', blendAmount(p));
		let blossomC = blendColor('#ffe419', '#FF0000', blendAmount(p));
        let blossomD = blendColor('#cfb914', '#FF0000', blendAmount(p));
		if (checkForFirstFrame(p)) {
			colorA = "#FFFFFF";
			colorB = "#FFFFFF";
			colorC = "#FFFFFF";
		...
		ctx.beginPath();
		ctx.moveTo(0, 0)
		ctx.arc(0, 0, p.radius, 0, Math.PI / 8, true);
		ctx.lineTo(0, 0)
		ctx.fill();
		ctx.closePath();
		...
		ctx.beginPath();
		ctx.moveTo(0, 0)
		ctx.arc(0, 0, p.radius * 0.66, 0, Math.PI / 8, true);
		ctx.lineTo(0, 0)
		ctx.fill();
		ctx.closePath();
		...
		for (let i = 0; i < 8; i++) {
			ctx.beginPath();
			ctx.moveTo(0, 0);
			ctx.quadraticCurveTo(p.radius * 0.5, p.radius * 0.25, p.radius * 0.75, 0);
			ctx.quadraticCurveTo(p.radius * 0.5, p.radius * -0.25, 0, 0);
			ctx.fill();
			ctx.stroke();
			ctx.closePath();
			ctx.rotate(Math.PI / 4);
		...
		ctx.lineWidth = p.radius * 0.075

		ctx.beginPath()
		ctx.arc(0, 0, p.radius * 0.1, 0, Math.PI * 2)
		ctx.fill();
		ctx.stroke();
		ctx.closePath();
	},
    LilypadProjectile: (p) => {
        petalRenderMap.Lilypad(p)
    },
    LilypadGiantProjectile: (p) => {
        petalRenderMap["Flowering Lilypad"](p)
    },
    Blossom: (p) => {
        ctx.lineWidth = p.radius * 0.15
		let blossomA = blendColor('#ff94c9', '#FF0000', blendAmount(p));
    ...
    Carapace: (p) => {
		...
		let outline = blendColor('#b15a3f', "#FF0000", Math.max(0, blendAmount(p)));
		let fill = blendColor('#dc704b', "#FF0000", Math.max(0, blendAmount(p)))
		if (checkForFirstFrame(p)) {
			outline = "#ffffff";
			fill = "#ffffff";
		}
		...
		ctx.fillStyle = fill;
		ctx.lineWidth = p.radius * 0.1826086956521739;


		//Body structure
		ctx.beginPath();
		ctx.ellipse(0, 0, p.radius * 0.8695652173913043, p.radius * 1.1130434782608696, 0, 0, Math.PI * 2);
		ctx.fill();
		ctx.stroke();
		ctx.closePath();
		//Patterns on body
		ctx.beginPath();
		ctx.lineTo(p.radius * -0.49, p.radius * -0.39);
		ctx.quadraticCurveTo(p.radius * 0, p.radius * -0.16, p.radius * 0.49, p.radius * -0.39)
		ctx.stroke();
		ctx.closePath();
    ...
    Thorax: (p) => {
        // ts pmo
        let outline = blendColor('#5a8f3f', "#228B22", Math.max(0, blendAmount(p)));
        let fill = blendColor('#7fbf4b', "#32CD32", Math.max(0, blendAmount(p)));
        ...
        // Main 
        ctx.beginPath();
        ctx.ellipse(0, 0, p.radius * 0.7, p.radius * 1.2, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.stroke();
        ctx.closePath();
        ...
        ctx.beginPath();
        ctx.moveTo(-p.radius * 0.6, -p.radius * 0.3);
        ctx.lineTo(p.radius * 0.6, -p.radius * 0.3);
        ctx.stroke();
        ctx.closePath();
        ...
        // Slight arc detail down center
        ctx.lineWidth = p.radius * 0.05;
        ctx.beginPath();
        ctx.moveTo(0, -p.radius * 0.9);
        ctx.quadraticCurveTo(p.radius * 0.2, 0, 0, p.radius * 0.9);
        ctx.stroke();
        ctx.closePath();
    },
    "Trinket of the Hivemind": (p) => {
        
        // main border

        ctx.fillStyle = blendColor("#ccb866", "#ff0000", blendAmount(p));
        ctx.strokeStyle = blendColor("#b8a756", "#ff0000", blendAmount(p));
        ...
        ctx.moveTo(p.radius, 0);
        for (let i = 0; i <= sides; i++) {
            ctx.lineTo(Math.cos(i*angle) * p.radius * 1.33, Math.sin(i*angle) * p.radius * 1.33);
            ctx.lineTo(Math.cos((i+.5)*angle) * p.radius * .45, Math.sin((i+.5)*angle) * p.radius * .45);
        };
        ctx.fill();
        ctx.stroke();
        ctx.closePath();
        ...
        ctx.fillStyle = blendColor("#d5c481", "#ff0000", blendAmount(p));
        ...
        for (let i = 0; i <= sides; i++) {
            ctx.lineTo(Math.cos(i*angle) * p.radius * 1.33 / 2, Math.sin(i*angle) * p.radius * 1.33 / 2);
            ctx.lineTo(Math.cos((i+.5)*angle) * p.radius / 2 * .45, Math.sin((i+.5)*angle) * p.radius / 2 * .45);
        };
        ctx.fill();
        ctx.closePath();
    },
    "Trinket of the Sea": (p) => {
        ...
        for (let i = 0; i <= sides; i++) {
            ctx.lineTo(Math.cos(i*angle) * p.radius, Math.sin(i*angle) * p.radius);
            ctx.lineTo(Math.cos((i+.5)*angle) * p.radius * .5, Math.sin((i+.5)*angle) * p.radius * .5);
        };
        ctx.fill();
        ctx.stroke();
        ctx.closePath();
        ...
        ctx.fillStyle = blendColor("#6b9ee1", "#ff0000", blendAmount(p));
        ...
        for (let i = 0; i <= sides; i++) {
            ctx.lineTo(Math.cos(i*angle) * p.radius / 2, Math.sin(i*angle) * p.radius / 2);
            ctx.lineTo(Math.cos((i+.5)*angle) * p.radius / 2 * .5, Math.sin((i+.5)*angle) * p.radius / 2 * .5);
        };
        ctx.fill();
        ctx.closePath();
    },
    "Trinket of the Wild": (p) => {
        
        // main border

        ctx.fillStyle = blendColor("#257831", "#ff0000", blendAmount(p));
        ctx.strokeStyle = blendColor("#1e6228", "#ff0000", blendAmount(p));
        ...
        for (let i = 0; i <= sides; i++) {
            ctx.lineTo(Math.cos(i*angle) * p.radius, Math.sin(i*angle) * p.radius);
            ctx.lineTo(Math.cos((i+.5)*angle) * p.radius * .66, Math.sin((i+.5)*angle) * p.radius * .66);
        };
        ctx.fill();
        ctx.stroke();
        ctx.closePath();
        ...
        ctx.fillStyle = blendColor("#4c9056", "#ff0000", blendAmount(p));
        ...
        for (let i = 0; i <= sides; i++) {
            ctx.lineTo(Math.cos(i*angle) * p.radius / 2, Math.sin(i*angle) * p.radius / 2);
            ctx.lineTo(Math.cos((i+.5)*angle) * p.radius / 2 * .66, Math.sin((i+.5)*angle) * p.radius / 2 * .66);
        };
        ctx.fill();
        ctx.closePath();
    },
    Plank: (p) => {
        ctx.fillStyle = blendColor("#b86c32", "#ff0000", blendAmount(p));
        ctx.strokeStyle = blendColor("#955728", "#ff0000", blendAmount(p));
        ...
        ctx.beginPath();
        ctx.moveTo(-0.63 * p.radius, -1.41 * p.radius);
        ctx.lineTo(0.55 * p.radius, -1.43 * p.radius);
        ctx.lineTo(0.57 * p.radius, -1.03 * p.radius);
        ctx.lineTo(0.45 * p.radius, -0.48 * p.radius);
        ctx.lineTo(0.59 * p.radius, -0.48 * p.radius);
        ctx.lineTo(0.63 * p.radius, 1.42 * p.radius);
        ctx.lineTo(0.28 * p.radius, 1.42 * p.radius);
        ctx.lineTo(0.17 * p.radius, 1.22 * p.radius);
        ctx.lineTo(0.17 * p.radius, 1.43 * p.radius);
        ctx.lineTo(-0.56 * p.radius, 1.42 * p.radius);
        ctx.lineTo(-0.62 * p.radius, 0.18 * p.radius);
        ctx.lineTo(-0.38 * p.radius, 0.16 * p.radius);
        ctx.lineTo(-0.61 * p.radius, 0.07 * p.radius);
        ctx.lineTo(-0.63 * p.radius, -1.41 * p.radius);
        ctx.fill();
        ctx.stroke();
        ctx.closePath();

        ctx.lineWidth = 0.12 * p.radius;

        ctx.beginPath();
        ctx.moveTo(-0.37 * p.radius, -0.98 * p.radius)
        ctx.quadraticCurveTo(-0.09 * p.radius, -1.33 * p.radius, 0.05 * p.radius, -0.99 * p.radius)
        ctx.quadraticCurveTo(-0.05 * p.radius, -0.63 * p.radius, -0.19 * p.radius, -0.8 * p.radius)
        ctx.quadraticCurveTo(-0.3 * p.radius, -0.89 * p.radius, -0.17 * p.radius, -0.96 * p.radius)
        ctx.quadraticCurveTo(-0.08 * p.radius, -0.98 * p.radius, -0.08 * p.radius, -0.9 * p.radius)
        ctx.stroke();
        ctx.closePath();
    },
    Carrot: (p) => {
        let leaf = blendColor("#39b54a", "#ff0000", blendAmount(p));
        let leafStroke = blendColor("#2e933c", "#ff0000", blendAmount(p));
        let carrot = blendColor("#f09148", "#ff0000", blendAmount(p));
        let carrotStroke = blendColor("#c2753a", "#ff0000", blendAmount(p));
        ...
        ctx.rotate(-Math.PI/2)
        ctx.beginPath();
        ctx.moveTo(-0.16 * p.radius, -0.9 * p.radius)
        ctx.quadraticCurveTo(0.06 * p.radius, -0.94 * p.radius, 0.25 * p.radius, -0.85 * p.radius)
        ctx.quadraticCurveTo(0.24 * p.radius, -1.05 * p.radius, 0.62 * p.radius, -1.15 * p.radius)
        ctx.quadraticCurveTo(0.6 * p.radius, -1.34 * p.radius, 0.22 * p.radius, -1.22 * p.radius)
        ctx.quadraticCurveTo(0.1 * p.radius, -1.86 * p.radius, -0.13 * p.radius, -1.78 * p.radius)
        ctx.quadraticCurveTo(-0.04 * p.radius, -1.38 * p.radius, -0.12 * p.radius, -1.23 * p.radius)
        ctx.quadraticCurveTo(-0.18 * p.radius, -1.34 * p.radius, -0.58 * p.radius, -1.41 * p.radius)
        ctx.quadraticCurveTo(-0.17 * p.radius, -1.03 * p.radius, -0.16 * p.radius, -0.9 * p.radius)
        ctx.fill();
        ctx.stroke();
        ctx.closePath();

        ctx.fillStyle = carrot;
        ctx.strokeStyle = carrotStroke;

        ctx.beginPath();
        ctx.moveTo(0, -p.radius * 0.8)
        ctx.bezierCurveTo(0.5 * p.radius, -0.77 * p.radius, 0.61 * p.radius, 0.2 * p.radius, 0, p.radius * 1.32)
        ctx.moveTo(0, -p.radius * 0.8)
        ctx.bezierCurveTo(-0.5 * p.radius, -0.77 * p.radius, -0.61 * p.radius, 0.2 * p.radius, 0, p.radius * 1.32)
        ctx.fill();
        ctx.stroke();
    ...
    CarrotProjectile: (p) => {
        petalRenderMap.Carrot(p)
    },
    "Shattered Relic of Wrath": (p) => {
        petalRenderMap.Basic(p)
    },
    "Reinforced Relic of Wrath": (p) => {
        petalRenderMap.Basic(p)
    },
    "Subset Relic of the Guardian": (p) => {
        petalRenderMap.Basic(p)
    },
    "Division Relic of the Guardian": (p) => {
        petalRenderMap.Basic(p)
    },
    "Guard Relic of the Guardian": (p) => {
        petalRenderMap.Basic(p)
    },
    "Knight Relic of the Guardian": (p) => {
        petalRenderMap.Basic(p)
    },
    "Aid Relic of Serenity": (p) => {
        petalRenderMap.Basic(p)
    },
    "Subliminal Relic of Serenity": (p) => {
        petalRenderMap.Basic(p)
    },
    "Barrier Relic of Serenity": (p) => {
        petalRenderMap.Basic(p)
    },
    Custom: (p, ...params) => {
