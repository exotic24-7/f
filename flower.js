...
const flowerInterpolateKeys =       ['headX',    'headY',    'radius',  'hp',  'baseX',  'baseY',   'petalDistance', 'beforeStreakHp', 'shield', 'isPoisoned', 'healingReduction'];
const flowerInterpolateMagnitudes = [0.072,       0.072,       0.44,     0.22,  0.088,    0.088,     0.2,           0.22,                 0.22,    0.088,            0.088];
...
class Flower {
    ...
    init(data){
        if (data.id == selfId){
            if (data.maxMana){
                if (data.maxMana.grace != undefined && data.maxMana.grace > 0){
                    GraceBar.init(data.maxMana.grace, data.mana.grace);
                }
                if (data.maxMana.time != undefined && data.maxMana.time > 0){
                    TimeBar.init(data.maxMana.time, data.mana.time);
                }
                if (data.maxMana.divergence != undefined && data.maxMana.divergence > 0){
                    DivergenceBar.init(data.maxMana.divergence, data.mana.divergence);
                }
            }
        }
        for(let key in data){
    ...
    drawProjectiles(){
        if(this.projectiles.length === 0 || (window.renderSquadPetals !== true && this.id !== window.selfId)){
            return;
        }
        for(let i = 0; i < this.projectiles.length; i++){
            if (!this.projectiles[i]) continue;
            if(toRender({x: this.projectiles[i].x, y: this.projectiles[i].y, radius: this.projectiles[i].radius}, window.camera) === true){
    ...
    drawPets(){
        for(let i = 0; i < this.pets.length; i++){
            ...
            if(toRender({x: this.pets[i].x, y: this.pets[i].y, radius: this.pets[i].radius * 4}, window.camera) === true){
                this.pets[i].draw();
            }
        }
        ...
        for(let i = 0; i < this.deadPets.length; i++){
            ...
            if(toRender({x: this.deadPets[i].x, y: this.deadPets[i].y, radius: this.deadPets[i].radius * 4}, window.camera) === true){
    ...
    draw(){
        ...
        for(let i = 0; i < this.deadProjectiles.length; i++){
            if (!this.deadProjectiles[i] || (window.renderSquadPetals !== true && this.id !== window.selfId)) continue;
        ...
        if (!(window.renderSquadPetals !== true && this.id !== window.selfId)) if(this.character === 'Tanksmith' && state === 'menu'){
            let barrelNum = 1;
            for(let i = 0; i < this.petals.length; i++){
                let petal = this.petals[i];
                if(toRender({x: petal.x, y: petal.y, radius: petal.radius}, window.camera) === true){
                    // let lastType = petal.type;
                    // petal.type = 'TanksmithPetal';
                    if(petal.subStackedId !== 0) {
                        barrelNum++;
                        continue;
                    }
                    ctx.translate(petal.render.x, petal.render.y);

                    let renderType = 'Normal';
                    if(['Rose','Dahlia','Leaf','Starfish','Shell'].includes(petal.type)) renderType = 'Heal';
                    else if(['Yucca'].includes(petal.type)) renderType = 'TankHeal';
                    else if(petal.type === 'Bud') renderType = 'Bud';
                    else if(petal.type === 'Bloom') renderType = 'Bloom';
                    else if(petal.type === 'Magnet') renderType = 'Magnet';
                    else if(petal.type === 'Egg' || petal.type === "Jellyfish Egg" || petal.type === "Neuroflare Egg") renderType = 'Egg';
                    
                    enemyRenderMap["Tanksmith"]({render: {radius: petal.radius * Math.sqrt(barrelNum), angle: petal.render.angle}, barrelNum, tsRenderType: renderType, ticksSinceLastDamaged: 1001, lastTicksSinceLastDamaged: 1001});
                    ctx.translate(-petal.render.x, -petal.render.y);
                    barrelNum = 1;
                    // petal.draw();
                }
        ...
        if(toRender({x: this.headX, y: this.headY, radius: this.radius}, window.camera) === true){
            this.drawFlower(this.render.headX, this.render.headY, this.radius, this.character);
        }

        //LIGHTNING
        if (!(window.renderSquadPetals !== true && this.id !== window.selfId))if (this.lightnings){
            if (this.lightnings.length > 0){
                this.lightnings = this.lightnings.filter((e) => time < (e.time+600)) //600ms time

                let prevLtnType = null
                for(let i of this.lightnings){

                    if (i.type !== prevLtnType) {
                        ctx.strokeStyle = ["#97f0ea", "#f0ee97", "#00fca4", "#a82d2d"][i.type];
                        prevLtnType = i.type;
                    }
                    
                    if (i.rarity >= 13) {
                        ctx.lineWidth = 9 * (2-(time-i.time)/700);
                    } else {
                        ctx.lineWidth = 5 * (2-(time-i.time)/700);
                    }
                    ...
                    if (i.rarity >= 13){
                        ctx.lineWidth = 18 * (2-(time-i.time)/700);
                        ctx.stroke();
                    }
        ...
        if (!(window.renderSquadPetals !== true && this.id !== window.selfId))if (this.blasts){
            if (this.blasts.length > 0){
                 this.blasts = this.blasts.filter((e) => time < (e.time+600))
                for(let i of this.blasts){
                    if (i.type == 0){
                        ctx.fillStyle = "#581e75"
                        ctx.globalAlpha = (0.115385-(time-i.time)/5200);
                    }
                    else if (i.type == 1){
                        ctx.fillStyle = "#e68612"
                        ctx.globalAlpha = (0.0555555-(time-i.time)/10800);
                    }
                    ctx.beginPath();
                    ctx.arc(i.x, i.y, i.radius + this.radius, 0, Math.PI * 2);
                    ctx.fill();
                    ctx.closePath();
                }
                ctx.globalAlpha = 1;
    ...
    drawFlower(x, y, radius, character='flower'){
        if (this.dead) return;
        if (this.dev){
            if(character === "Tanksmith"){
                ctx.fillStyle = blendColor('#790000', '#FF0000', Math.max(0, 1 - this.ticksSinceLastDamaged / 166));
                ctx.strokeStyle = blendColor('#550000', '#FF0000', Math.max(0, 1 - this.ticksSinceLastDamaged / 166));
                ctx.lineWidth = 4;
            } else {
            ...
            if (this.render.healingReduction < 0.999){
                ctx.fillStyle = blendColor(ctx.fillStyle, '#ffffff', Math.max(0, 1-this.render.healingReduction));
                ctx.strokeStyle = blendColor(ctx.strokeStyle, '#ffffff', Math.max(0, 1-this.render.healingReduction));
            }
            ...
            if(character === 'Tanksmith'){
                ctx.translate(x,y);
                // barrel
                ctx.rotate(this.render.angle);
                
                ctx.lineWidth = 5;
                ctx.fillStyle = blendColor('#bfbfbf', "#FF0000", Math.max(0, 1 - this.ticksSinceLastDamaged / 166));
                ctx.strokeStyle = blendColor('#868686', "#FF0000", Math.max(0, 1 - this.ticksSinceLastDamaged / 166));
                for(let i = 0; i < /*e.barrelNum*/8; i++){
                    ctx.beginPath();
                    ctx.roundRect(0, -radius * 0.23, radius * 1.3, radius * 0.23 * 2, 2);
                    ctx.fill();
                    ctx.stroke();
                    ctx.closePath();
                    ctx.rotate(Math.PI * 2 / 8/*e.barrelNum*/);
                }
    ...
    static drawDeadFlower(x, y, radius){
        if (this.dev) {
            ...
            let eyecenter = { x: x + radius / 2.5 + radius * 0.05, y: y + radius * 5 / 13.5 };
            ...
            eyecenter = { x: x - radius / 2.5, y: y - radius * 5 / 23.5 + radius * -0.15 };
